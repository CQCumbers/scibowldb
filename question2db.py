...
root_path = os.path.expanduser(r'~/Google Drive/Projects/SciBowl_Questions/')
# Folder names after root_path (except last text folder) are listed in source, along with name of file
#paths = glob(root_path+'16Exchange/*')
paths = glob(root_path+'00Regs')
...
for folder in paths:
    rounds_path = folder+'/text' # A folder of OCR'd Science Bowl sample rounds
    ...
    for filename in os.listdir(rounds_path):
        with open(os.path.join(rounds_path, filename), encoding='utf8') as file:
            ...
            for match in regex.findall(file.read()):
                if regex == question_re.nats98_regex: # special case for 98Nats regex output
                    if match[1] == 'ASTR':
                        category = 'ASTRONOMY'
                    elif match[1] == 'BIOL':
                        category = 'BIOLOGY'
...
if testing:
    json.dump(questions, open('questions_test.json', 'w+'), indent=4, separators=(',', ': '), sort_keys=True)  # make JSON pretty looking
else:
